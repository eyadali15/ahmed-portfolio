...
function CTASection() {
  ...
  const ctaTitleStyle = getElementStyle('home', 'ctaTitle');
  const ctaDescStyle  = getElementStyle('home', 'ctaDescription');
    ...
    <section className="cta-section" style={sectionStyle}>
      <div className="container-main max-w-[700px]" style={{ textAlign: l.align as Align }}>
        ...
        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.1 }}
          className="font-[var(--font-heading)] text-3xl md:text-4xl lg:text-5xl leading-tight text-white" style={{ marginBottom: e.titleMarginBottom, ...ctaTitleStyle }}>
          {c.title}
        </motion.h2>
        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }}
          className="text-white/50 max-w-md text-base leading-relaxed" style={{ paddingTop: e.descriptionPaddingTop, marginBottom: e.descriptionMarginBottom, margin: l.align === 'center' ? `0 auto ${e.descriptionMarginBottom}px` : undefined, ...ctaDescStyle }}>{c.description}</motion.p>
