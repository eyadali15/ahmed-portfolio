...
const icons: Record<string, React.ReactNode> = {
  vimeo: <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.977 6.416c-.105 2.338-1.739 5.543-4.894 9.609-3.268 4.247-6.026 6.37-8.29 6.37-1.409 0-2.578-1.294-3.553-3.881L5.322 11.4C4.603 8.816 3.834 7.522 3.01 7.522c-.179 0-.806.378-1.881 1.132L0 7.197c1.185-1.044 2.351-2.084 3.501-3.128C5.08 2.701 6.266 1.984 7.055 1.91c1.867-.18 3.016 1.1 3.447 3.838.465 2.953.789 4.789.971 5.507.539 2.45 1.131 3.674 1.776 3.674.502 0 1.256-.796 2.265-2.385 1.004-1.589 1.54-2.797 1.612-3.628.144-1.371-.395-2.061-1.614-2.061-.574 0-1.167.121-1.777.391 1.186-3.868 3.434-5.757 6.762-5.637 2.473.06 3.628 1.664 3.493 4.797l-.013.01z"/></svg>,
...
export default function Contact() {
  ...
  const info = {
    ...
    content: {
      ...
      facebookUrl: (cms?.contact?.info as any)?.facebookUrl || '',
      whatsappUrl: cms?.contact?.info?.whatsappUrl || staticContent.info.content.whatsappUrl,
      instagramUrl: (cms?.contact?.info as any)?.instagramUrl || '',
  ...
  const socialLinks = [
    { key: 'instagram', label: 'Instagram', href: gen.instagram || info.content.instagramUrl || ft.instagramUrl || '' },
    { key: 'facebook', label: 'Facebook', href: gen.facebook || info.content.facebookUrl || ft.facebookUrl || '' },
    { key: 'vimeo', label: 'Vimeo', href: gen.vimeo || info.content.vimeoUrl || ft.vimeoUrl || '' },
    { key: 'whatsapp', label: 'WhatsApp', href: gen.whatsapp || info.content.whatsappUrl || '' },
  ].filter(l => l.href);
  ...
  const formHeaderLabelStyle = mergeLayoutStyle({}, 'contact', 'formHeaderLabel');
  const formStyle     = mergeLayoutStyle({}, 'contact', 'form');
  const fieldGroupStyle = mergeLayoutStyle({}, 'contact', 'formFieldGroup');
  const labelsStyle   = mergeLayoutStyle({}, 'contact', 'formLabels');
  const inputsStyle   = mergeLayoutStyle({}, 'contact', 'formInputs');
  const submitBtnStyle = mergeLayoutStyle({}, 'contact', 'formSubmitBtn');
  ...
  return (
    <PageTransition>
      <section className="min-h-screen flex flex-col">
        ...
        <div className="relative h-[70vh] min-h-[500px] w-full overflow-hidden flex items-center justify-center contact-hero">
          ...
          <div className="relative z-10 container-main flex flex-col items-center text-center max-w-2xl mx-auto px-4">
            ...
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
              className="contact-title font-[var(--font-heading)] text-3xl md:text-4xl lg:text-5xl leading-tight mb-5 text-white"
              style={{ ...titleStyle, ...headerTitleStyle }}>
            ...
            <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}
              className="contact-description text-white/50 text-base leading-relaxed max-w-md text-center"
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4">
          <div className="w-full max-w-2xl mx-auto" style={{ paddingTop: 48 }}>
            ...
            <div className="contact-info" style={infoStyle}>
              ...
              >
                ...
                <a href={`mailto:${info.content.email}`}
                  className="group flex items-center gap-2.5 text-sm text-white/50 hover:text-[var(--color-accent)] transition-colors">
              ...
              <motion.div
                ...
                className="contact-form-header flex items-center justify-center gap-3 mb-6" style={formHeaderLabelStyle}>
                <div className="w-8 h-px bg-[var(--color-accent)]" />
                <span className="contact-form-header-label text-[10px] uppercase tracking-[0.35em] text-[var(--color-accent)]">{(cms?.contact?.info as any)?.formHeaderLabel || 'Send a Message'}</span>
                <div className="w-8 h-px bg-[var(--color-accent)]" />
              </motion.div>
