...
export default function Contact() {
  ...
  /* ── Build social links array (only show if URL is set) ── */
  const socialLinks = [
    { key: 'instagram', label: 'Instagram', href: info.content.instagramUrl },
    { key: 'facebook', label: 'Facebook', href: info.content.facebookUrl },
    { key: 'vimeo', label: 'Vimeo', href: info.content.vimeoUrl },
                ...
                <div className="flex items-center gap-3">
                  {socialLinks.map((link) => (
                    <a key={link.key} href={link.href} target="_blank" rel="noopener noreferrer" title={link.label}
                      className="text-white/25 hover:text-[var(--color-accent)] transition-colors duration-300">
                      {icons[link.key]}
                    </a>
                  ))}
                </div>
