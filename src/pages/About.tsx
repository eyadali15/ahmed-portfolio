...
export default function About() {
  ...
  const servicesLabel = (cms as any)?.about?.servicesLabel || 'Services';
  const servicesTitle = (cms as any)?.about?.servicesTitle || 'What I Do';
  ...
  const servicesTitleStyle   = mergeLayoutStyle({}, 'about', 'servicesTitle');
  const servicesGridStyle    = mergeLayoutStyle({}, 'about', 'servicesGrid');
  const servicesCardStyle    = mergeLayoutStyle({}, 'about', 'servicesCard');

  
  const sHeroLabel    = getElementStyle('about', 'heroLabel');
  const sHeroTitle    = getElementStyle('about', 'heroTitle');
  const sIntroText1   = getElementStyle('about', 'introText1');
  const sIntroText2   = getElementStyle('about', 'introText2');
  const sGalleryLabel = getElementStyle('about', 'galleryLabel');
  const sGalleryTitle = getElementStyle('about', 'galleryTitle');
  const sSvcLabel     = getElementStyle('about', 'svcLabel');
  const sSvcTitle     = getElementStyle('about', 'svcTitle');
  const sTlLabel      = getElementStyle('about', 'tlLabel');
  const sTlTitle      = getElementStyle('about', 'tlTitle');
  const sQuoteText    = getElementStyle('about', 'quoteText');
  const sQuoteAuthor  = getElementStyle('about', 'quoteAuthor');
    ...
    <PageTransition>
      ...
      <section className="relative h-[50vh] min-h-[350px] w-full overflow-hidden flex items-end about-hero" style={box(hb.layout)}>
        ...
        <div className="relative z-10 container-main pb-12" style={{ textAlign: hb.layout.align as Align }}>
          <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}
            className="text-[10px] uppercase tracking-[0.4em] text-[var(--color-accent)]" style={{ marginBottom: hb.elements.labelMarginBottom, ...sHeroLabel }}>{hb.content.label}</motion.p>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}
            className="font-[var(--font-heading)] text-4xl md:text-5xl lg:text-6xl text-white" style={{ marginBottom: hb.elements.titleMarginBottom || undefined, ...sHeroTitle }}>
            {hb.content.title}
          </motion.h1>
        </div>
      </section>
      ...
      <section ref={heroRef} className="about-intro" style={box(intro.layout)}>
        <div className="container-main" style={{ textAlign: intro.layout.align as Align }}>
          <div className={intro.layout.align === 'center' ? 'max-w-2xl mx-auto' : 'max-w-2xl'}>
            ...
            <p className="about-reveal text-white/60 text-base leading-relaxed max-w-xl" style={{ marginBottom: intro.elements.text1MarginBottom, margin: intro.layout.align === 'center' ? `0 auto ${intro.elements.text1MarginBottom}px` : `0 0 ${intro.elements.text1MarginBottom}px`, ...sIntroText1 }}>{intro.content.text1}</p>
            <p className="about-reveal text-white/60 text-base leading-relaxed max-w-xl" style={{ marginBottom: intro.elements.text2MarginBottom || undefined, margin: intro.layout.align === 'center' ? '0 auto' : undefined, ...sIntroText2 }}>{intro.content.text2}</p>
          </div>
        </div>
      </section>
      )}
        ...
        <section className="border-t border-[var(--color-border)] about-gallery" style={box(gallery.layout)}>
          <div className="container-main" style={{ textAlign: gallery.layout.align as Align }}>
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
              <p className="text-[10px] uppercase tracking-[0.3em] text-[var(--color-accent)]" style={{ marginBottom: gallery.elements.labelMarginBottom, ...sGalleryLabel }}>{gallery.content.label}</p>
              <h2 className="font-[var(--font-heading)] text-2xl md:text-3xl" style={{ marginBottom: gallery.elements.titleMarginBottom, ...sGalleryTitle }}>{gallery.content.title}</h2>
            </motion.div>
      ...
      <section className="border-t border-[var(--color-border)] about-services" style={servicesSectionStyle}>
        <div className="container-main">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}
            ...
            <span className="text-[10px] uppercase tracking-[0.35em] text-[var(--color-accent)]" style={sSvcLabel}>{servicesLabel}</span>
          ...
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.1 }}
            className="services-title font-[var(--font-heading)] text-2xl md:text-3xl lg:text-4xl mb-12" style={{ ...servicesTitleStyle, ...sSvcTitle }}>
            {servicesTitle}
          </motion.h2>
      ...
      <section className="border-t border-[var(--color-border)] about-timeline" style={box(tl.layout)}>
        <div className="container-main" style={{ textAlign: tl.layout.align as Align }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
            <p className="text-[10px] uppercase tracking-[0.3em] text-[var(--color-accent)]" style={{ marginBottom: tl.elements.labelMarginBottom, ...sTlLabel }}>{tl.content.label}</p>
            <h2 className="font-[var(--font-heading)] text-2xl md:text-3xl" style={{ marginBottom: tl.elements.titleMarginBottom, ...sTlTitle }}>{tl.content.title}</h2>
          </motion.div>
      ...
      <section className="border-t border-[var(--color-border)] about-quote" style={box(q.layout)}>
        <div className="container-main max-w-[700px]" style={{ textAlign: q.layout.align as Align }}>
          <motion.div initial={{ opacity: 0, scale: 0.97 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            ...
            <blockquote className="font-[var(--font-heading)] text-2xl md:text-3xl italic text-white leading-relaxed" style={{ marginBottom: q.elements.quoteMarginBottom, ...sQuoteText }}>{q.content.text}</blockquote>
            <cite className="text-xs text-white/30 uppercase tracking-[0.2em] not-italic" style={sQuoteAuthor}>— {q.content.author}</cite>
          </motion.div>
        </div>
      </section>
      )}
