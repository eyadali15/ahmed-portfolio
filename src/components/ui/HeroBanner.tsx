...
export default function HeroBanner() {
  ...
  const name1Style = getElementStyle('home', 'name1');
  const name2Style = getElementStyle('home', 'name2');
  const subtitleStyle = getElementStyle('home', 'subtitle');
      ...
      <div className="relative z-[3] h-full flex flex-col items-center justify-center px-6" style={{ textAlign: (s as any).heroTextAlign as Align || 'center' }}>
        <div ref={titleRef} style={{ marginBottom: (s as any).heroTitleMarginBottom || 36 }}>
          <h1 className="font-[var(--font-heading)] text-4xl sm:text-5xl md:text-6xl lg:text-7xl leading-[1] tracking-tight">
            <span className="hero-word inline-block text-white" style={name1Style}>{c.name1}</span>
            <span className="hero-word inline-block ml-3 md:ml-4 text-[var(--color-accent)]" style={name2Style}>{c.name2}</span>
          </h1>
        </div>
        <div ref={subtitleRef} style={{ textAlign: (s as any).heroTextAlign as Align || 'center' }}>
          <p className="text-xs md:text-sm tracking-[0.35em] uppercase text-white/70" style={{ marginBottom: (s as any).heroSubtitleMarginBottom || 20, ...subtitleStyle }}>{c.subtitle}</p>
