...
function initStyleEngine() {
  const customizableIds = {
    ...
    'about-hero-label':    'about.styles.heroLabel',
    ...
    'about-gallery-label': 'about.styles.galleryLabel',
    'about-gallery-title': 'about.styles.galleryTitle',
    'about-svc-label':     'about.styles.svcLabel',
    'about-svc-title':     'about.styles.svcTitle',
    'about-tl-label':      'about.styles.tlLabel',
    'about-tl-title':      'about.styles.tlTitle',
    'about-quote-text':    'about.styles.quoteText',
    'about-quote-author':  'about.styles.quoteAuthor',
...
function populateStyleFields() {
  const maps = {
    ...
    'about-hero-label': config.about?.styles?.heroLabel,
    ...
    'about-gallery-label': config.about?.styles?.galleryLabel,
    'about-gallery-title': config.about?.styles?.galleryTitle,
    'about-svc-label': config.about?.styles?.svcLabel,
    'about-svc-title': config.about?.styles?.svcTitle,
    'about-tl-label': config.about?.styles?.tlLabel,
    'about-tl-title': config.about?.styles?.tlTitle,
    'about-quote-text': config.about?.styles?.quoteText,
    'about-quote-author': config.about?.styles?.quoteAuthor,
...
function readStyleFields() {
  ...
  config.about.styles.heroLabel = readStyle('about-hero-label');
  ...
  config.about.styles.galleryLabel = readStyle('about-gallery-label');
  config.about.styles.galleryTitle = readStyle('about-gallery-title');
  config.about.styles.svcLabel = readStyle('about-svc-label');
  config.about.styles.svcTitle = readStyle('about-svc-title');
  config.about.styles.tlLabel = readStyle('about-tl-label');
  config.about.styles.tlTitle = readStyle('about-tl-title');
  config.about.styles.quoteText = readStyle('about-quote-text');
  config.about.styles.quoteAuthor = readStyle('about-quote-author');
...
function populateAllForms() {
  ...
  if (c.about.services) {
    setVal('about-svc-label', c.about.servicesLabel || 'Services');
    setVal('about-svc-title', c.about.servicesTitle || 'What I Do');
    renderServiceItems();
  }
  ...
  setVal('contact-form-label', c.contact.info.formHeaderLabel || 'Send a Message');
  setVal('contact-form-btn', c.contact.info.formButtonText || 'Send Message');
...
function readAllForms() {
  ...
  config.about.servicesTitle = getVal('about-svc-title');
  readServiceItems();
  ...
  config.contact.info.formHeaderLabel = getVal('contact-form-label');
  config.contact.info.formButtonText = getVal('contact-form-btn');
...
function renderServiceItems() {
  ...
  if (!container) return;
  const items = config.about.services || [];
  container.innerHTML = items.map((item, i) => `
    <div class="project-list-item" style="flex-direction:column;align-items:stretch;gap:0.5rem;">
      <div style="display:flex;gap:0.5rem;">
        <input type="text" value="${esc(item.title)}" id="svc-title-${i}" placeholder="Service Title" style="flex:1;">
        <button class="btn-admin btn-admin--sm btn-admin--danger" onclick="removeServiceItem(${i})">✕</button>
      </div>
      <textarea id="svc-desc-${i}" placeholder="Description" rows="2">${esc(item.description)}</textarea>
      <input type="text" value="${esc((item.tags || []).join(', '))}" id="svc-tags-${i}" placeholder="Tags (comma separated)">
    </div>
  `).join('');
}

function readServiceItems() {
  config.about.services = (config.about.services || []).map((item, i) => ({
    ...item,
    title: getVal(`svc-title-${i}`) || item.title,
    description: getVal(`svc-desc-${i}`) || item.description,
    tags: (getVal(`svc-tags-${i}`) || '').split(',').map(s => s.trim()).filter(Boolean)
  }));
}

window.removeServiceItem = function(i) {
  readServiceItems();
  config.about.services.splice(i, 1);
  renderServiceItems();
};

window.addServiceItem = function() {
  readServiceItems();
  if (!config.about.services) config.about.services = [];
  config.about.services.push({ icon: '🎬', title: '', description: '', tags: [], svgPath: 'M18 4l2 4h-3l-2-4h-2l2 4h-3l-2-4H8l2 4H7L5 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V4h-4z' });
  renderServiceItems();
};
